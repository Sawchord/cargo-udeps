use cargo::{
    core::{
        compiler::{DefaultExecutor, Executor, RustcTargetData, Unit},
        dependency,
        manifest::Target,
        package_id::PackageId,
        resolver::{
            features::{CliFeatures, ForceAllTargets},
            HasDevUnits,
        },
        shell::Shell,
        Package, Resolve, Verbosity, Workspace,
    },
    ops::Packages,
    util::{
        command_prelude::{ArgMatchesExt, CompileMode, ProfileChecking},
        ...
        for (to_pkg, deps) in resolve.deps(from) {
            ...
            if let Some(to_lib) = to_pkg.targets().iter().find(|t| t.is_lib()) {
                ...
                for dep in deps {
                    ...
                    if r.is_some() {
                        shell.warn(format!(
                            "duplicate package mentioned in toml {}. {:?}",
                            to_pkg.package_id(),
                            r
                        ))?;
                    }
